...
alter table public.arnaud_photo_palette_colours
  drop constraint arnaud_photo_palette_colours_rank_check;
alter table public.arnaud_photo_palette_colours
  add constraint arnaud_photo_palette_colours_rank_check check (rank between 1 and 7);

-- New imports must publish a complete seven-colour palette.
...
-- Reanalysis replaces a published photograph's palette in one transaction.
...
as $$
begin
  if p_algorithm is distinct from 'kmeans-rgb-v1'
    or p_algorithm_iterations is distinct from 10
    or p_sample_longest_side is distinct from 96
    or p_palette_size is distinct from 7
    or jsonb_typeof(p_colours) is distinct from 'array'

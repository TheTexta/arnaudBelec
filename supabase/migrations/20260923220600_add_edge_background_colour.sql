...
alter table public.arnaud_photographs
  add column background_hex text check (background_hex ~ '^#[0-9a-fA-F]{6}$');

-- The extra argument changes each function's signature. Remove the old write
-- paths so imports cannot omit the analysed background colour.
drop function public.arnaud_publish_photograph(
  uuid, text, text, integer, integer, date, text, text, integer,
  text, smallint, smallint, smallint, timestamptz, jsonb
);
drop function public.arnaud_replace_photo_palette(
...
begin
  if p_storage_path !~ '^uploads/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(jpg|png|webp)$'
    ...
    or p_background_hex is null
    or p_background_hex !~ '^#[0-9a-fA-F]{6}$'
  ...
  insert into public.arnaud_photographs (
    ...
    captured_at, title, alt_text, sort_order, background_hex
  ) values (
    p_photograph_id, p_storage_path, p_filename, p_image_width, p_image_height,
    p_captured_at, p_title, p_alt_text, p_sort_order, p_background_hex
  );
...
create function public.arnaud_replace_photo_palette(
  p_photograph_id uuid,
  p_algorithm text,
  p_algorithm_iterations smallint,
  p_sample_longest_side smallint,
...
begin
  ...
  update public.arnaud_photographs
  set background_hex = p_background_hex
...
revoke all on function public.arnaud_publish_photograph(
  uuid, text, text, integer, integer, date, text, text, integer,
  text, smallint, smallint, smallint, timestamptz, text, jsonb
...
revoke all on function public.arnaud_replace_photo_palette(
  uuid, text, smallint, smallint, smallint, timestamptz, text, jsonb
) from public, anon, authenticated;
grant execute on function public.arnaud_replace_photo_palette(
  uuid, text, smallint, smallint, smallint, timestamptz, text, jsonb
) to service_role;
